...
class STGCNDecoder(nn.Module):
    ...
    def __init__(
        ...
        if self.st_gcn_layers:
            final_channels = layer_configs[min(num_layers, len(layer_configs)) - 1][1]
        else:
            final_channels = hidden_dim

        # Output projection to keypoint coordinates
        self.output_proj = nn.Conv2d(final_channels, keypoint_dim, kernel_size=1, bias=True)
