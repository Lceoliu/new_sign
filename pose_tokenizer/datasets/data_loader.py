...
def collate_pose_sequences(batch: list) -> Dict[str, torch.Tensor]:
    ...
    for sample in batch:
        pose_sequences.append(sample['poses'])
        masks.append(sample['mask'])
        metadata.append(sample['metadata'])

    # Pad to max length in batch
    max_len = max(p.shape[0] for p in pose_sequences)
    num_keypoints = pose_sequences[0].shape[1]
    keypoint_dim = pose_sequences[0].shape[2]

    batched_poses = torch.zeros(len(pose_sequences), max_len, num_keypoints, keypoint_dim, dtype=pose_sequences[0].dtype)
    batched_masks = torch.zeros(len(pose_sequences), max_len, dtype=torch.bool)

    for i, (pose_seq, mask) in enumerate(zip(pose_sequences, masks)):
        seq_len = pose_seq.shape[0]
        batched_poses[i, :seq_len] = pose_seq
        if mask.shape[0] == seq_len:
            batched_masks[i, :seq_len] = mask
        else:
            batched_masks[i, :seq_len] = True
...
def create_data_loaders(
    ...
    for split in ['train', 'val', 'test']:
        ...
        dataset = PoseDataset(
            ...
            clip_mode=clip_mode if split == 'train' else "full",
            window_T=window_T or sequence_length,
...
def compute_dataset_statistics(data_path: str) -> Dict[str, Any]:
    ...
    for idx in indices:
        try:
            sample = dataset[idx]
            pose_seq = sample['poses'].numpy()  # (T, N, D)
