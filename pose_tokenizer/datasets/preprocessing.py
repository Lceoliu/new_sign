...
class PosePreprocessor:
    ...
    def process_raw_poses(
        ...
        poses = self._sanitize_invalid(poses)

        # Fill missing keypoints
        if fill_missing:
            poses = self._fill_missing_keypoints(poses, interp_max_gap=interp_max_gap)

        # Smooth trajectories
        if smooth:
            poses = self._smooth_poses(poses, window_size=smooth_window, polyorder=smooth_polyorder)

        # Root-relative transform
        if root_relative:
            poses = self._root_relative(poses, root_joint=root_joint)

        # Scale normalization
        if scale_norm:
            poses = self._scale_normalize(poses, mode=scale_mode)
    ...
    def _fill_missing_keypoints(self, poses: np.ndarray, interp_max_gap: int = 3) -> np.ndarray:
        ...
        for n in range(N):  # For each keypoint
            for d in range(min(2, D)):  # Only interpolate x, y coordinates
                ...
                if len(valid_indices) >= 2:
                    ...
                    # Fill gaps only if within max gap
                    filled_trajectory = poses[:, n, d].copy()
                    all_indices = np.arange(T)
                    interp_values = interp_func(all_indices)

                    missing = ~valid_mask
                    if interp_max_gap is None:
                        filled_trajectory[missing] = interp_values[missing]
                    else:
                        ...
                        for i, is_missing in enumerate(missing):
                            if is_missing and start is None:
                                start = i
                            if (not is_missing or i == T - 1) and start is not None:
                                end = i if not is_missing else i + 1
                                if end - start <= interp_max_gap:
                                    filled_trajectory[start:end] = interp_values[start:end]
                                start = None
    ...
    def _smooth_poses(self, poses: np.ndarray, window_size: int = 5, polyorder: int = 2) -> np.ndarray:
        ...
        # Apply moving average smoothing (or Savitzky-Golay if window is large)
        for n in range(N):
            for d in range(min(2, D)):  # Only smooth x, y coordinates
                ...
                if window_size >= 5:
                    try:
                        ...
                        smoothed = savgol_filter(trajectory, window_size, polyorder, mode='nearest')
                    except Exception:
                        smoothed = trajectory
                else:
    ...
    def _sanitize_invalid(self, poses: np.ndarray) -> np.ndarray:
        """Replace NaN/Inf with zeros."""
        poses = poses.copy()
        invalid_mask = ~np.isfinite(poses)
        if invalid_mask.any():
            poses[invalid_mask] = 0.0
        return poses

    def _root_relative(self, poses: np.ndarray, root_joint: int = 0) -> np.ndarray:
        """Make poses root-relative by subtracting root joint per frame."""
        if root_joint < 0 or root_joint >= poses.shape[1]:
            return poses
        poses = poses.copy()
        root = poses[:, root_joint, :2]  # (T, 2)
        poses[:, :, :2] = poses[:, :, :2] - root[:, None, :]
        return poses

    def _scale_normalize(self, poses: np.ndarray, mode: str = "shoulder_width") -> np.ndarray:
        """Scale normalize poses per frame."""
        poses = poses.copy()
        T, N, D = poses.shape

        if mode == "shoulder_width" and N > 12:
            left = poses[:, 11, :2]
            right = poses[:, 12, :2]
        ...
        if scale is None or np.all(scale == 0):
            if self.bones:
                bone_pairs = np.array(self.bones, dtype=int)
                i_idx = bone_pairs[:, 0]
                j_idx = bone_pairs[:, 1]
                vec = poses[:, i_idx, :2] - poses[:, j_idx, :2]
                lengths = np.linalg.norm(vec, axis=-1)
                scale = lengths.mean(axis=1)
            else:
                scale = np.ones(T, dtype=np.float32)

        scale = np.maximum(scale, 1e-6)
        poses[:, :, :2] = poses[:, :, :2] / scale[:, None, None]
        return poses
