...
def create_dataloader(config: PoseTokenizerConfig, split: str = 'test'):
    ...
    if config.eval_num_samples > 0 and len(dataset) > config.eval_num_samples:
        dataset = Subset(dataset, range(config.eval_num_samples))

    batch_size = config.eval_batch_size if config.eval_batch_size > 0 else config.batch_size
...
def evaluate_reconstruction_quality(model, dataloader, device, logger):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            if mask is not None:
                ...
                token_lengths.extend(mask.sum(dim=1).cpu().tolist())
            else:
                valid_samples = poses.shape[0] * poses.shape[1]
                token_lengths.extend([poses.shape[1]] * poses.shape[0])

            if bones is None:
                try:
                    from pose_tokenizer.models.encoder import MediaPipeGraph
                    bones = MediaPipeGraph(num_keypoints=pred_pos.shape[2]).edges
                except Exception:
                    bones = []
            ...
            if pred_pos.shape[1] > 1:
                ...
                tgt_vel = target_pos[:, 1:] - target_pos[:, :-1]
                if mask is not None:
                    vel_mask = (mask[:, 1:] & mask[:, :-1]).unsqueeze(-1).unsqueeze(-1)
                    pred_vel = pred_vel * vel_mask
                    tgt_vel = tgt_vel * vel_mask
                    vel_count = vel_mask.sum().item()
                else:
                    vel_count = pred_vel.numel()
                vel_mae = torch.mean(torch.abs(pred_vel - tgt_vel)).item()
                total_vel_mae += vel_mae * max(vel_count, 1)
                total_vel_frames += max(vel_count, 0)

            # Acceleration MAE
            if pred_pos.shape[1] > 2:
                pred_acc = pred_pos[:, 2:] - 2 * pred_pos[:, 1:-1] + pred_pos[:, :-2]
                tgt_acc = target_pos[:, 2:] - 2 * target_pos[:, 1:-1] + target_pos[:, :-2]
                if mask is not None:
                    acc_mask = (mask[:, 2:] & mask[:, 1:-1] & mask[:, :-2]).unsqueeze(-1).unsqueeze(-1)
                    pred_acc = pred_acc * acc_mask
                    tgt_acc = tgt_acc * acc_mask
                    acc_count = acc_mask.sum().item()
                else:
                    acc_count = pred_acc.numel()
                acc_mae = torch.mean(torch.abs(pred_acc - tgt_acc)).item()
                total_acc_mae += acc_mae * max(acc_count, 1)
                total_acc_frames += max(acc_count, 0)

            # Bone length MAE
            if bones:
                bone_pairs = torch.tensor(bones, device=pred_pos.device, dtype=torch.long)
                i_idx = bone_pairs[:, 0]
                j_idx = bone_pairs[:, 1]
                pred_vec = pred_pos[:, :, i_idx] - pred_pos[:, :, j_idx]
                tgt_vec = target_pos[:, :, i_idx] - target_pos[:, :, j_idx]
                pred_len = torch.sqrt(torch.sum(pred_vec ** 2, dim=-1) + 1e-8)
                tgt_len = torch.sqrt(torch.sum(tgt_vec ** 2, dim=-1) + 1e-8)
                if mask is not None:
                    bone_mask = mask.unsqueeze(-1)
                    pred_len = pred_len * bone_mask
                    tgt_len = tgt_len * bone_mask
                bone_mae = torch.mean(torch.abs(pred_len - tgt_len)).item()
                total_bone_mae += bone_mae * poses.shape[0]
    ...
    if total_vel_frames > 0:
        metrics['vel_l1'] = total_vel_mae / total_vel_frames
    if total_acc_frames > 0:
        metrics['acc_l1'] = total_acc_mae / total_acc_frames
    if total_samples > 0:
        metrics['bone_len_mae'] = total_bone_mae / total_samples

    if token_lengths:
        token_lengths = np.array(token_lengths)
        metrics['mean_L'] = float(np.mean(token_lengths))
        metrics['p95_L'] = float(np.percentile(token_lengths, 95))
...
def evaluate_tokenization_quality(model, dataloader, device, logger):
    ...
    for q, usage in codebook_usage.items():
        ...
        dead_threshold = 1e-5
        usage_full = np.zeros(codebook_size, dtype=np.float64)
        for idx, count in usage.items():
            if 0 <= idx < codebook_size:
                usage_full[idx] = count
        usage_full = usage_full / max(usage_full.sum(), 1e-8)
        dead_ratio = float(np.sum(usage_full < dead_threshold) / codebook_size)

        utilization_metrics[f'codebook_{q}_utilization'] = utilization
        utilization_metrics[f'codebook_{q}_entropy'] = entropy
        utilization_metrics[f'codebook_{q}_perplexity'] = perplexity
        utilization_metrics[f'codebook_{q}_dead_ratio'] = dead_ratio
...
def main():
    ...
    # Also save a lightweight metrics.json for quick inspection
    ...
    if 'vel_l1' in reconstruction_metrics:
        print(f"VEL L1: {reconstruction_metrics['vel_l1']:.6f}")
    if 'acc_l1' in reconstruction_metrics:
        print(f"ACC L1: {reconstruction_metrics['acc_l1']:.6f}")
    if 'bone_len_mae' in reconstruction_metrics:
        print(f"BONE LEN MAE: {reconstruction_metrics['bone_len_mae']:.6f}")
